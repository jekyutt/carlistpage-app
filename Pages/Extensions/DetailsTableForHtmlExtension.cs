...
namespace Loppprojekt.Pages.Extensions {
    public static class DetailsTableForHtmlExtension {
        ...
        internal static IList<object> createString<TModel, TResult>(IHtmlHelper<TModel> htmlHelper, Expression<Func<TModel, IList<TResult>>> expression,
            Expression<Func<TResult, object>>[] properties) where TModel : PageModel
        {
            var htmlStrings = new List<object>();
            var f = expression.Compile();
            var items = f(htmlHelper.ViewData.Model);
            if (items.Count <= 0) return htmlStrings;
